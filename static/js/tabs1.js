...
function append_tab(group_id, data, id="") {
  append_to_ul(
    "group_" + group_id,
  ...
  append_to_ul("group_" + group_id, ``, undefined, id + "_line", "line");
  ...
  for (const group of config_file) {
    ...
    append_to_ul(
      ...
      `
        ...
        <div class="line"></div>
        <ul class="tabs_items" id="group_${group_id}"></ul>
    ...
    for (const el of group.items) {
      try {
        ...
        append_tab(group_id, TABS[TABS.length - 1], TABS[TABS.length - 1].id);
